...
- **一个巨大而模糊的工作——全新项目或巨型功能构建，一个会话装不下** → **`/wayfinder`**，这里认知要求最高的流程。当从此处到目的地的路还看不见时，它会在 issue tracker 上绘制一张由**决策工单**组成的**共享地图**，然后一次解决一个——产出的是**决策，而非交付物**——直到迷雾被推开、道路清晰。`/grill-with-docs` 打磨的是你能在一次会话中装下的想法，而 wayfinder 面向的是你装不下的那种——而且它更慢、更密集，所以只留给这种情况，永远不要用在边界清晰的功能上。
...
- **`/domain-modeling`** — 打磨项目的*领域*语言：挑战模糊的术语，消解一词多义（"account" 同时承担三个含义），把难以逆转的决定记录为 ADR。它是 `/grill-with-docs` 用来保持 `CONTEXT.md` 为一份干净词汇表的主动纪律。
...
- **`/grill-me`** — 与 `/grill-with-docs` 同样不留情面的访谈，但**无状态**：它不在本地保存任何东西，也不构建 `CONTEXT.md`。当你**不在工作目录中**时使用它——打磨一个计划、一份设计、一段文字，任何底下没有 repo 的东西。如果你在工作目录中，就用 `/grill-with-docs`：它运行同样的访谈并留下书面记录，所以严格来说它更好。
- **`/grilling`** — 访谈原语本身：轮次、前沿、事实是 agent 的工作而决策是你的。`/grill-me` 和 `/grill-with-docs` 是两个具名入口，`/wayfinder` 和 `/improve-codebase-architecture` 也都在内部运行它。只有当你想要不带任何包装的访谈时才直接使用它。
