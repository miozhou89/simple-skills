...
**照原文展示菜单**——简洁，每个选项都来自上面的列表。丢弃仅在明确要求时发生（见下）。等他们回答；集成与否是他们的决定。

## 步骤 5：执行选择

**选项 1 和丢弃路径都先切到主仓库根目录：**
...
等待**这个精确的**确认词。收到之后，切到主仓库根目录（步骤 5 开头那段），然后清理 worktree（步骤 6），再强制删除分支：
...
| "他们看起来对这个功能收工了——我提议丢弃吧" | 菜单照原文，不多不少。丢弃仅在明确要求时发生。 |
| "'嗯，删掉吧'算确认了" | 只有输入 `discard` 才授权删除。 |
| "PR 已经开了，worktree 现在是碍事的垃圾" | PR 反馈要在那个 worktree 里修。它得留到工作落地为止。 |
| "另外那个 worktree 看着像过期的——我顺手也清了" | 只清理 `.worktrees/` / `worktrees/` 下的 worktree，其余属宿主环境。 |
| "合并结果的失败大概是偶发的" | 合并结果失败即停下；排查期间分支与 worktree 原地不动。 |
